...
以降の`<TARGET>`は、プロジェクトルートからの対象パスへ置き換えてください。

## 文書契約を用意する

対象本文を読み、`<TARGET>.intent.yml`を確認してください。
...
node "${CLAUDE_PLUGIN_ROOT}/scripts/claude-review-cli.mjs" prepare "<TARGET>" > "${CLAUDE_PROJECT_DIR}/.jp-docs-harness/work/review-packet.json"
```
