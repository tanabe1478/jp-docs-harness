...
export function resolveDocumentScope({ projectDir, target = "" }) {
  ...
  if (repositoryRoot === null) {
    if (targetIsDirectory) {
      throw new Error(
        "Gitリポジトリを特定できません。ディレクトリ全体の検査にはGitリポジトリが必要です。Markdownファイルを1件指定してください",
      );
    }
    // Gitに属さないMarkdownは、1件指定に限りファイルのあるディレクトリを境界にする。
    // 文書契約と.jp-docs-harness/はそのディレクトリへ置かれる。
    return { cwd: gitStart, files: [path.basename(absoluteTarget)] };
  }
