import path from "node:path";
import { formatHarnessReport, hasBlockingFindings } from "./core/human-report.mjs";
import { createHarnessReport } from "./core/report.mjs";
import { resolveTargetPatterns } from "./core/target-files.mjs";
import { contractGate } from "./gates/contract.mjs";
import { evidenceGate } from "./gates/evidence.mjs";
import { freshnessGate } from "./gates/freshness.mjs";
import { READING_LOAD_MODES, readabilityGate } from "./gates/readability.mjs";
import { semanticResultGate } from "./gates/semantic-result.mjs";
import { BOLD_POLICIES, styleGate } from "./gates/style.mjs";
import { surfaceGate } from "./gates/surface.mjs";

export async function runHarness({
  textlint,
  yaml,
  Ajv,
  cwd,
  configFilePath,
  nodeModulesDir,
  intentSchemaPath = path.join(path.dirname(configFilePath), "schemas", "intent.schema.json"),
  reviewResultSchemaPath = path.join(
    path.dirname(configFilePath),
    "schemas",
    "review-result.schema.json",
  ),
  files = [],
  reviewMode = "manual",
  failOn = "error",
  boldPolicy = "forbid",
  readingLoad = "off",
  gates = [surfaceGate, contractGate, styleGate, readabilityGate, evidenceGate, freshnessGate, semanticResultGate],
}) {
  if (!["manual", "contracted", "strict"].includes(reviewMode)) {
    throw new Error(`不明なreview modeです: ${reviewMode}`);
  }
  if (!["error", "warning"].includes(failOn)) {
    throw new Error(`不明な--fail-onです: ${failOn}`);
  }
  if (!BOLD_POLICIES.includes(boldPolicy)) {
    throw new Error(`不明な--boldです: ${boldPolicy}`);
  }
  if (!READING_LOAD_MODES.includes(readingLoad)) {
    throw new Error(`不明なreading loadモードです: ${readingLoad}`);
  }

  const patterns = resolveTargetPatterns({ cwd, files });
  const documentsByPath = new Map();
  const findings = [];

  for (const gate of gates) {
    const result = await gate.run({
      textlint,
      yaml,
      Ajv,
      cwd,
      configFilePath,
      nodeModulesDir,
      intentSchemaPath,
      reviewResultSchemaPath,
      patterns,
      reviewMode,
      boldPolicy,
      readingLoad,
      documents: [...documentsByPath.values()],
      findings: [...findings],
    });

    for (const document of result.documents ?? []) documentsByPath.set(document.path, document);
    findings.push(...(result.findings ?? []));
  }

  const report = createHarnessReport({ documents: [...documentsByPath.values()], findings });

  return {
    report,
    humanOutput: formatHarnessReport(report),
    hasFindings: report.findings.length > 0,
    hasBlocking: hasBlockingFindings(report, { failOn }),
  };
}
