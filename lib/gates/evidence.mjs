...
export const evidenceGate = {
  id: "evidence",
  async run({ cwd, documents, yaml, Ajv, intentSchemaPath }) {
    const findings = [];
    ...
    for (const document of documents) {
      ...
      for (const source of packet.grounding.sources) {
        ...
        findings.push(createEvidenceFinding(document.path, source));
      }
    }
...
function createEvidenceFinding(document, source) {
