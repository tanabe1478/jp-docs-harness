...
文書検査は利用者が明示的に依頼したときだけ動きます。Claude Codeではスラッシュコマンドのほか、「この文書を検査して」「レビューして」のような会話での依頼でも起動します。Stop hookやpiの`agent_settled`による自動検査は行いません。作業を勝手に中断せず、複数リポジトリの親ディレクトリから起動した場合にも、意図しないリポジトリを検査しないためです。
...
意味レビューは表現の警戒も行います。根拠が近くに示されていない評価の断定、宛先が読めない規範、事実を隠す比喩、執筆時の指示への応答が本文へ漏れた文（脈絡のない除外宣言など）を、行番号と言い換え案を添えた助言として報告します。根拠を先に示した評価は正当と扱い、本文の自動修正はしません。定型で判定できる表現は`check-docs`が決定論的に警告します。
